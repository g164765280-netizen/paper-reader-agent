# -*- coding: utf-8 -*-
"""概念题 PaperQA2 通路 v3：本地 Qwen3-8B(vLLM) + 本地 bge-m3 embedding + 权威概念锚点。

在 v2 基础上（RAG 调参 + prompt 强化，模型仍是本地 8B），新增：
- 把 data/resources/concepts.json 里筛出的 31 条「权威概念词条」（有来源+可验证+无待查证）
  拼成一份「领域权威概念库」文档，随论文一起 aadd 进 Docs。
- 完全不动 paperqa 的检索/摘要/答题流程，只是多喂一份权威知识文档，
  让 8B 答题时能同时看到「论文证据 + 权威定义」，对冲摘要环节丢信息/写反术语的问题。

用法：
  /media/sdb1/gzj/support/llm/envs/paperqa/bin/python paperqa_concept_v3.py
"""
...
OUT = Path("/media/sdb1/gzj/support/llm/rscd/concept_answers_v3.jsonl")
